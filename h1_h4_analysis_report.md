...
> v2 变更要点：H3 在新增的 144 条注入式负例上 100% 被 verifier 捕获（128 reject + 16 review），同时该实验暴露并修复了两个 verifier 规则 bug。H3 由 v1 的 ⚠️"形式通过" 升级为 ✅"具备分辨力"。残余缺口集中在 (a) 文档池规模与多样性（仍 5 篇、无真 10-K / 无扫描件）、(b) H4 步数方差与 rule-based baseline 缺失、(c) **自然分布**上的人工 GT confusion matrix 仍未做、(d) 负例为人工注入式 mutation，与"teacher 自然出错"的分布尚不等价。
...
| OCR text CER（人工抽检 10 段） | ❌ 未给出 | — | H1 |
| BM25 top-3 retrieval rate（10 query） | ✅ 部分 | retrieval check 10/10 | H1 |
| parse_table 行列结构正确率 | ❌ 未给出 | expectation 包含但未独立报告 | H1 |
| crop bbox 有效性（人工抽检） | ❌ 未给出 | 提供 contact sheet，未量化 | H1 |
...
> v2 指标缺口分布：H1 三项、H2 一项、**H3 五项中 4 项已被注入式负例覆盖、1 项 (sufficiency) 仅在拒答场景压力测试**、H4 两项。H3 的剩余风险已从 "未检验" 收窄为 "仅检验注入式分布、未检验自然分布"。
...
| 设计指标 | 现状 | 风险 |
|---|---|---|
| OCR CER ≤5%（人工抽检 10 段） | 没有 CER 数字，仅"output text 与 expectation 一致" | expectation 检查比对的是关键 token 而非全段文本，可能放过空格/换行/连字符层面错误 |
| parse_table 行列结构 ≥70% | 没有独立的"行列对齐"指标 | 100% expectation 通过依赖于设计了易解析表格作为 expectation 目标，存在选择偏差 |
| crop bbox 命中率 ≥85% | contact sheet 仅可视化，未给出命中率数字 | 无法支撑"任意 bbox 都能正确裁剪"的论文 claim |
...
**风险**：如果不在论文中明确"adjusted evaluator 的判定规则"，审稿人可能将 100% 视为评估宽松化。需要补充：
...
- **风险**：这等同于在论文中默认"required tool 与 acceptable tool 可互换"。需补充：
  - 每个失败 seed 上 teacher 走了哪条 acceptable path
  - 缺失的 required tool 是哪一个（很可能是 `parse_table`，被 `read_page` 直接读出来的文本替代）
  - 这种替代是否会损失"结构化证据 ID"——如果 H3 依赖 cell-level 引用，那么跳过 `parse_table` 的轨迹应判失败而非通过
...
verification 与 table_lookup 各 8 条轨迹 = 2 个 seed × 4 rollout，**事实上等价于只测了 2 个例子**，不能支撑独立 claim。
...
本次实验的"成功"建立在 GPT-4o / Gemini-2.5-Flash 这种 SOTA closed-source teacher 上。若 H5 阶段 student（如 Qwen2.5-VL-7B）无法复现这种"格式 100%、终止 100%、direct answer 0%"的行为，则 H1-H4 的成功不构成对训练 pipeline 的端到端证据。
...
| # | 实验 | 目的 |
|---|---|---|
| P1-1 | 扩文档池：补 5+ 篇真 SEC 10-K（EDGAR）、3+ 篇扫描 PDF、2+ 篇含彩色背景/手写表格 PDF | 压力测试 H1 失败模式探针；检验降级触发器 |
| P1-2 | OCR CER 人工抽检（每篇 10 段，共 ≥150 段） | 给出 09 设计要求的 CER 数字 |
| P1-3 | parse_table 行列对齐人工抽检（每篇 5 表，共 ≥75 表） | 给出独立的 table 结构正确率 |
| P1-4 | crop bbox 命中率人工抽检（≥50 个 bbox） | 量化"任意 bbox 都能正确裁剪"的支撑度 |
| P1-5 | H2 strict path 失败 per-seed breakdown | 区分"等价工具替代"与"真实证据缺失" |
| P1-6 | H2 strict vs adjusted 差距的失败案例分析（15 条） | 判断 18.75pp 是否全为格式归一化 |
| P1-7 | Teacher 成本/时延对照表（GPT-4o vs Gemini） | 指导大规模 rollout 的 teacher 选型 |
| P1-8 | 工具调用合理性人工 1-5 分评分（采样 30 条） | 补全 09 设计指标 |

### P2 — 加固 H4

| # | 实验 | 目的 |
|---|---|---|
| P2-1 | 定义并实现 rule-based template baseline（按任务类型写死序列） | 给出"与模板的偏离度"数字 |
| P2-2 | 步数分布 std 与 per-task 步数直方图 | 直接回答 09 门槛 std ≥1.5 |
| P2-3 | seeds 扩至 100+ 后重新统计 unique sequence / search query | 检验"扩规模后 sequence 多样性是否同比增长" |
| P2-4 | 引入第三个异质 teacher（Claude 4.5 Sonnet / Qwen-VL-Max） | 跨 teacher 家族多样性 |
| P2-5 | 加入需要 5+ 步的复杂 seed（多跳 cross-page、verify 后 re-search、多表 numeric） | 提高步数方差，覆盖长链推理 |

### P3 — 启动 H5

| # | 前置 | 实验 |
|---|---|---|
| P3-1 | P0 + P2-3 完成 | 准备 ≥200 条 filtered 轨迹 + ≥200 条 answer-only 对照，按 09 Exp-5 设计执行 SFT |
| P3-2 | P3-1 完成 | 在 ≥10 个 held-out 文档问题上对比工具调用率 / 工具格式正确率 / 答案 F1 / 证据引用率 / refusal 行为 |

---

## 9. 风险与降级触发器

v2 已触发的正面信号：DocVerify++ 在注入分布上 caught_bad = 100%，**未触发任何降级路径**。下列信号若在 P0-4/5/6 中出现，仍需按 09 预设降级方案处理：
...
|---|:---:|:---:|---|
| C3-a：DocEnv-lite 提供可复现环境 | 强（5 篇 100%） | H1 | 扩文档池后维持 ≥90% 成功率（P1-1） |
...
> **现阶段可写入论文**：H1（部分）、H2、H3（在注入式负例上 100% 召回，需明确报告为 "injected-mutation negative control"）、H4。  
...
| **H3 负例 144/144 caught (128 reject + 16 review)** | summary §H3 Negative-Control Experiment |
| **H3 6 类 failure taxonomy** | summary §H3 Negative-Control Experiment |
| **H3 verifier bug 修复 (verify label / table final-answer)** | summary §H3 Negative-Control Experiment / Fixed Issue table |
| H3 人工审查输出文件 | `data/h3/negative_v4/negative_manual_review.md`；`negative_manual_review.jsonl`；`negative_manual_labels_template.jsonl`；`negative_docverify_review.md` |
...
H1-H4 pilot smoke test 已走通；v2 新增的 144 条注入式负例对照让 H3 从 v1 的 "形式通过、未真正验证" 升级为 "在注入分布上 100% caught，且暴露并修复了 2 个 verifier 规则 bug"。这意味着：
- "DocVerify++ 是一个有效的过滤器" 这条 claim 已具备**首次直接证据**，可在 paper 中以 *injected-mutation negative control* 的语义谨慎宣称
- 但要把它升级为 "在 teacher 自然产出分布上也 precision/recall 双高"，仍需 P0-4（自然分布 GT）+ P0-5（review/reject 阈值校准）+ P0-6（更丰富的 mutation）

建议下一步优先级：

1. **P0-4 自然分布人工 GT 标注**（60-100 条）— H3 升级为强证据的必经路径；**注入式 GT ≠ 自然 GT**，论文若混用会被审稿人挑战
2. **P0-5 review vs reject 阈值校准** — 16 条拒答负例全部 review 而非 reject 的现象需要在论文中解释或修正
3. **P1-1/2/3/4** 扩文档池 + 人工抽检 — 把 H1 从 "5 篇 100%" 升级为 "~15 篇含失败模式"
4. **P2-1/2** 定义 rule-based baseline + 步数 std — 回应 "unique 12.5% < 设计 50%" 的质疑
5. **P3 启动 H5** — 在 P0-4 完成、verifier 自然分布 precision 确认后再启动，避免对照集污染
